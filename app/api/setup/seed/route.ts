...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!existingPlan) {
      const totalTarget = 12_00_00_000;
    ...
    return NextResponse.json({
      ok: true,
      message: "Setup complete. Your team and demo projects are loaded.",
      dgmLogin: { email: "dgm@ap-pms.local", password: DEFAULT_PASSWORD },
      teamMembersLoaded: staffUsers.length,
    });
  } catch (err) {
